...
class NameMarshallerSpec  extends Specification {
  ...
  "The name marshaller functionality" should {
    ...
    "allow for the creation of a valid NameMarshaller" in {
      val marshaller = NameMarshallerFactory.getNameMarshaller("org.eigengo.monitor.output.metrics.TestNameMarshaller", "prefix.")
    ...
    "build an appropriate name when given an aspect and tags" in {
      ...
      name must be equalTo s"${prefix}my-actor.queue-size"
      ...
      internalName must be equalTo s"${prefix}internal.some-akka-actor.queue-size"
    }
  }
}
